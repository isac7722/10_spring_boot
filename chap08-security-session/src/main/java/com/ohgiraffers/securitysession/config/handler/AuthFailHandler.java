...
* 우리는 AuthenticationFailureHandler 구현해야 하지만 기존에 구현이 되었는 SimpleUrlAuthenticationFailureHandler를 상속받아
...
public class AuthFailHandler extends SimpleUrlAuthenticationFailureHandler {
    ...
    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response, AuthenticationException exception) throws IOException, ServletException {
        ...
        if (exception instanceof BadCredentialsException){ // BadCredentialsException 오류는 사용자의 아이디가 DB에 존재하지 않는 경우 비밀번호가 맞지 않는 경우 발생한다.
